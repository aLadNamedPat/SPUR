...
class GridEnv(ParallelEnv):
    ...
    def __init__(self, 
        ...
        self.numCenters = num_centers
        self.max_timesteps = max_timesteps

        self.gridTracker = GridTracker(grid_size, bound)
        self.envGrid = GridWorld(grid_size, num_centers)

        self.agent_positions = self.GSL()

        self.possible_agents = [f"agent{i}" for i in range(self.numAgents)]
        self.agents = self.possible_agents[:]

        #Current timestep of the environment
        self.curr_step = 0
    ...
    def reset(
        ...
        observations = {
            f"agent{a}" : { 
                "observation" : {
                    "eReward" : self.gridTracker.update((self.agent_positions[a][0], self.agent_positions[a][1]))[0], #Estimated expected reward of the map
                    "pGrid" : self.gridTracker.update((self.agent_positions[a][0], self.agent_positions[a][1]))[1], #Probabiity grid of the map
                    "agent" : self.gridTracker.update((self.agent_positions[a][0], self.agent_positions[a][1]))[2], #Gridspace of where the agent is located
                },
    ...
    def step(self, actions):
        ...
        observations = { #Doesn't save the timestep that events occur intrinsicly (This occurs at the moment that the agents meet)
            f"agent{a}" : {
                "observation" : {
                    "eReward" : self.gridTracker.update((self.agent_positions[a][0], self.agent_positions[a][1], self.curr_step))[0], #Estimated expected reward of the map
                    "pGrid" : self.gridTracker.update((self.agent_positions[a][0], self.agent_positions[a][1], self.curr_step))[1], #Probabiity grid of the map
                    "agent" : self.gridTracker.update((self.agent_positions[a][0], self.agent_positions[a][1], self.curr_step))[2], #Gridspace of where the agent is located
                },
                "agent_mask" : action_mask[a]
            }
            for a in range(len(self.agents))
    ...
    def observation_space(
        ...
        return Dict({
            "eReward" : Box(low = 0, high = self.bound, shape=(self.gridSize, self.gridSize)),
            "pGrid" : Box(low = 0, high = 1, shape = (self.gridSize, self.gridSize)),
            "agent" : Box(low = 0, high = 1, shape = (self.gridSize, self.gridSize), type = np.int128)
        })
